...
stoi_scores_ds = zeros(size(desired_SNR_db));               % Stoi scores for ds beamformer
stoi_scores_mwdr = zeros(size(desired_SNR_db));             % Stoi scores for mwdr beamformer
stoi_scores_mwdr_Rn_cheat = zeros(size(desired_SNR_db));    % Stoi scores for mwdr beamformer using cheated Rn
stoi_scores_wiener = zeros(size(desired_SNR_db));           % Stoi scores for mcw filter
stoi_scores_wiener_Rs_cheat = zeros(size(desired_SNR_db));  % Stoi scores for mcw filter using cheated Rs
...
for index = 1:length(desired_SNR_db)
    ...
    stoi_scores_mwdr(index) = stoi(y_mwdr, x_signal_only(:, 1), fs);
    stoi_scores_mwdr_Rn_cheat(index) = stoi(y_mwdr_cheat, x_signal_only(:, 1), fs);
    stoi_scores_wiener(index) = stoi(y_wiener, x_signal_only(:, 1), fs);
    stoi_scores_wiener_Rs_cheat(index) = stoi(y_wiener_cheat, x_signal_only(:, 1), fs);
    stoi_scores_ds(index) = stoi(y_delay_sum, x_signal_only(:, 1), fs);
...
figure;
hold on;
plot(desired_SNR_db, stoi_scores_ds, '-x', 'DisplayName', 'DS');
plot(desired_SNR_db, stoi_scores_mwdr, '-o', 'DisplayName', 'MWDR');
plot(desired_SNR_db, stoi_scores_mwdr_Rn_cheat, '-s', 'DisplayName', 'MWDR + Rn cheat');
plot(desired_SNR_db, stoi_scores_wiener, '-*', 'DisplayName', 'Wiener');
plot(desired_SNR_db, stoi_scores_wiener_Rs_cheat, '-d', 'DisplayName', 'Wiener + Rs cheat');

hold off;

xlabel('SNR (dB)');
ylabel('STOI Score');
title('Comparison of STOI Scores');
